...
export class NatsConsumerService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly streamName = 'facebook';
  ...
  private async connect() {
    ...
    this.logger.info('Connected to NATS server for Facebook consumer');
  }
  ...
  private async setupConsumer() {
    try {
      ...
      this.logger.info(
        `Facebook consumer setup complete for ${this.subjectName}`,
      );
    } catch (error) {
      this.logger.error('Error setting up Facebook consumer:', error);
    }
  }
  ...
  async close() {
    if (this.natsConnection) {
      ...
      this.logger.info('Facebook NATS connection closed');
    }
  }
}
